...
import { ArrowUpRight, AudioLines, Volume2, VolumeX, Sparkles, Utensils, Leaf, Share2 } from 'lucide-react';
...
export default function Home(){
 ...
 async function shareResult(){
  if(!result)return;
  const text=`${t.sharePrefix} ${foodName(result,language)} — ${t.referencePrice} ${priceLabel(result.price,language,true)}`;
  try{
   if(navigator.share){await navigator.share({title:document.title,text});return}
   await navigator.clipboard.writeText(text);setShared(true);setTimeout(()=>setShared(false),2000);
